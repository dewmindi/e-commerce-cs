...
const FeaturedProjects = async () => {
    let projects = await getPortfolioByService('web-development').catch(() => []);
                        ...
                        {[...projects, ...projects].map((project, index) => (
                            <div key={index} className="w-[350px] md:w-[400px] mx-4 flex-shrink-0">
                                <BackgroundGradient className="rounded-[22px] max-w-sm p-2 bg-black">
                                    ...
                                    <p className="text-sm px-2 text-neutral-600 dark:text-neutral-400">{project.description}</p>
                                    {project.weblink && (
                                        <Link href={project.weblink} passHref target="_blank">
